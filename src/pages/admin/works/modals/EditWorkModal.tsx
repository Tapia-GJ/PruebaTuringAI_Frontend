...
import { updateWork } from '../../../../api/works.api';
...
export function EditWorkModal({ isOpen, onClose, onSuccess, work }: EditWorkModalProps) {
  ...
  useEffect(() => {
    if (work && isOpen) {
      ...
      setPublishYear(work.publishYear ? work.publishYear.toString() : '');
      setAuthorId(work.authorId.toString());
      
      // Extraemos los IDs de los géneros actuales de la obra
      if (work.workGenres) {
        setSelectedGenreIds(work.workGenres.map(wg => wg.genreId));
      } else {
        setSelectedGenreIds([]);
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateWork(work.id, {
        ...
        genreIds: selectedGenreIds // Siempre lo mandamos para que pueda limpiar si deselecciona todos
      });
      onSuccess();
      handleClose();
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Editar Obra">
